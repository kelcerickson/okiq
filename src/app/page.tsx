...
export default function Home() {
  ...
  const resultColors = {
    W: { bg: '#AAFF00', text: '#1A1A1A' },
    L: { bg: '#FF4444', text: '#fff' },
    D: { bg: '#888', text: '#fff' },
  }
      ...
      <div className="body">
        <div className="home-hero">
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <Link href="/roster" className="btn btn-ghost btn-sm">Roster</Link>
          ...
          <div key={m.id} className="mc-card" style={{ cursor: 'default' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
              <div style={{ flex: 1 }}>
                ...
                {m.our_score !== null && m.our_score !== undefined && (
                  <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 24, fontWeight: 800, color: '#1A1A1A' }}>{m.our_score}</span>
                    <span style={{ fontSize: 16, color: '#888' }}>—</span>
                    <span style={{ fontSize: 24, fontWeight: 800, color: '#1A1A1A' }}>{m.their_score}</span>
                  </div>
                )}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  {!m.finished && <Link href={`/match/${m.id}/live`} className="btn btn-neon btn-sm">Resume</Link>}
                  {m.finished && <Link href={`/match/${m.id}/live`} className="btn btn-sm" style={{ background: '#1E1E1E', color: '#AAFF00', border: '1px solid rgba(170,255,0,0.3)', fontSize: 12 }}>Edit</Link>}
            ...
            {m.join_code && (
              <div style={{ fontSize: 12, color: '#AAA', marginTop: 8 }}>
                Join code: <strong style={{ color: '#AAFF00', fontSize: 13 }}>{m.join_code}</strong>
              </div>
            )}
          </div>
        ))}
