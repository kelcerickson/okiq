...
interface Player { id: string; name: string; jersey_number: string | null }
interface MatchPlayer { id: string; player_id: string; current_pos: string | null; player: Player }
interface PosEntry { id: string; match_player_id: string; position: Position; active: boolean }
interface Stat { id: string; position_entry_id: string; stat_key: string; value: number }

export default function LiveMatch() {
  const { id } = useParams<{ id: string }>()
  const router = useRouter()
  const [match, setMatch] = useState<{ opponent: string; match_date: string; location: string | null; join_code: string } | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    if (mp) {
      ...
      if (mpIds.length) {
        ...
        if (pe) {
          setPosEntries(pe)
          const peIds = pe.map((e: PosEntry) => e.id)
          if (peIds.length) {
  ...
  async function setPosition(pos: Position) {
    if (!activeMp) return
    // Deactivate all current entries for this match_player
    ...
    // Check if entry for this position already exists
    let existing = myEntries.find(e => e.position === pos)
    if (!existing) {
      const { data } = await supabase.from('position_entries').insert({ match_player_id: activeMp.id, position: pos, active: true }).select().single()
      existing = data as PosEntry
      // Init stats
      if (existing) {
        const defs = STAT_DEFS[pos]
        const statsToInsert = defs.flatMap(s => s.type === 'rate'
          ? [{ position_entry_id: existing!.id, stat_key: s.key + '_hit', value: 0 }, { position_entry_id: existing!.id, stat_key: s.key + '_miss', value: 0 }]
  ...
  async function adjustStat(entryId: string, statKey: string, delta: number) {
    const current = stats.find(s => s.position_entry_id === entryId && s.stat_key === statKey)
    const newVal = Math.max(0, (current?.value || 0) + delta)
    await supabase.from('stats').upsert({ position_entry_id: entryId, stat_key: statKey, value: newVal })
    setStats(prev => {
    ...
    <div className="shell">
      ...
      <div className="body">
        <div className="mbanner">
          ...
          <div className="lbadge"><div className="ldot" style={{ width: 5, height: 5, borderRadius: '50%', background: '#000' }} />Live</div>
        </div>
        ...
        <div style={{ marginBottom: 13, display: 'flex', flexWrap: 'wrap', gap: 2 }}>
          {matchPlayers.map(mp => (
            <div key={mp.player_id}
              ...
              {mp.current_pos && <span className={`pp pp-${mp.current_pos}`} style={{ fontSize: 10, padding: '1px 5px' }}>{mp.current_pos}</span>}
            </div>
          ))}
        </div>
        ...
        {activeMp && (
          <div className="card">
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 13 }}>
              ...
              <div>
                <div style={{ fontWeight: 500, fontSize: 15 }}>
                  ...
                  {activeMp.player.jersey_number && <span style={{ fontSize: 12, color: 'var(--muted)' }}> #{activeMp.player.jersey_number}</span>}
                </div>
                <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
                  {activeMp.current_pos
                    ? <span className={`pp pp-${activeMp.current_pos}`}>{POSITION_LABELS[activeMp.current_pos as Position]}</span>
                    : 'No position set'}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14 }}>
              {(['F', 'M', 'D', 'G'] as Position[]).map(pos => (
                <button key={pos} className={`pbtn ${activeMp.current_pos === pos ? 'a' + pos : ''}`}
                  onClick={() => setPosition(pos)}>{POSITION_LABELS[pos]}</button>
              ))}
            </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(155px,1fr))', gap: 9, marginTop: 9 }}>
                  {activeDefs.map(s => {
                    if (s.type === 'rate') {
                      ...
                      return (
                        <div key={s.key} className="sblk">
                          <div style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 3, fontWeight: 500 }}>{s.label}</div>
                          <div style={{ fontSize: 26, fontWeight: 500, color: 'var(--neon)', lineHeight: 1, marginBottom: 3 }}>{rate}</div>
                          <div style={{ fontSize: 11, color: 'var(--dim)', marginBottom: 8 }}>{tot > 0 ? `${hit}/${tot} attempts` : ''}</div>
                          {[{ key: s.key + '_hit', label: s.hitL! }, { key: s.key + '_miss', label: s.missL! }].map(row => (
                            <div key={row.key} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                              <span style={{ fontSize: 11, color: 'var(--muted)', width: 64, flexShrink: 0 }}>{row.label}</span>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                <button className="cb minus" onClick={() => adjustStat(activeEntry.id, row.key, -1)}>−</button>
                                <span style={{ fontSize: 15, fontWeight: 500, minWidth: 24, textAlign: 'center' }}>
                                  {stats.find(st => st.position_entry_id === activeEntry.id && st.stat_key === row.key)?.value || 0}
                                </span>
                    ...
                    return (
                      <div key={s.key} className="sblk">
                        <div style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 3, fontWeight: 500 }}>{s.label}</div>
                        <div style={{ fontSize: 26, fontWeight: 500, color: 'var(--neon)', lineHeight: 1, marginBottom: 8 }}>{val}</div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 8 }}>
                          <button className="cb minus" onClick={() => adjustStat(activeEntry.id, s.key, -1)}>−</button>
                          <span style={{ fontSize: 15, fontWeight: 500, minWidth: 24, textAlign: 'center' }}>{val}</span>
            ...
            {prevEntries.length > 0 && (
              <>
                ...
                {prevEntries.map(e => (
                  <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 0', borderBottom: '1px solid var(--b3)', fontSize: 12, color: 'var(--muted)' }}>
